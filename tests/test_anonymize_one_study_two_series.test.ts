...
describe("patient", () => {
  ...
  beforeEach(() => {
    dataset1 = loadInstance(1, 1, 1);
    dataset2 = loadInstance(1, 1, 2);
    anonymizer = new Anonymizer();
  });

  it.sequential("should anonymize patient and study attributes the same", async () => {
  ...
  it.sequential("should anonymize series and instance attributes differently", async () => {
    const elementPaths = [
      // series
    ...
    for (const elementPath of elementPaths) {
      if (elementPath.length == 2) {
        ...
        expect(value1, elementPath[1]).not.toEqual(value2);
      } else {
        const value1 = dataset1[elementPath[0]][elementPath[1]].Value[0][elementPath[2]].Value[0];
        const value2 = dataset2[elementPath[0]][elementPath[1]].Value[0][elementPath[2]].Value[0];
